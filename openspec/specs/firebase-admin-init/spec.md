...
This capability provides single, idempotent initialization of the Firebase Admin app, including private-key newline normalization, Auth emulator detection outside production, a hard refusal to run against the emulator in production, and fail-fast configuration validation when service account credentials are missing.
