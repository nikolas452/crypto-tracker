...
This capability provides `requireAuth`, the middleware that parses the `Authorization` header, verifies the Bearer token through the injected `TokenVerifier`, and populates `req.auth` and `req.user` for downstream handlers — together with the `Request` type augmentation, the `getUser(req)` helper, and the guarantees that tokens are accepted only from the `Authorization` header and are never logged.
