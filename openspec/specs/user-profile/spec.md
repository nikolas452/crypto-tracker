...
This capability defines the `users` collection and just-in-time profile provisioning: creating the profile the first time a valid token appears for an unknown Firebase uid, handling the concurrent-first-request race, synchronizing `email`/`emailVerified` from the token, throttling `lastSeenAt` updates, and keeping authenticated requests to at most one read and one write against `users`.
